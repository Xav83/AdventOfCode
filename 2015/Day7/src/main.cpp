...
public:
  ...
  void addNewWire(const Wire &wire) {
    auto found = std::any_of(
        std::begin(wires), std::end(wires),
        [&wire](const auto &wireElement) { return wire == wireElement; });
    if (!found) {
...
public:
  And(Wire &input1_, Wire &input2_, Wire &output_)
      : Operation(output_), input_wire(&input1_), input2(input2_) {}
  And(Signal input1_, Wire &input2_, Wire &output_)
      : Operation(output_), input_signal(input1_), input2(input2_) {}
  ~And() = default;
  ...
  bool isExecuted() const override { return hasBeenExecuted; }
...
public:
  Or(Wire &input1_, Wire &input2_, Wire &output_)
      : Operation(output_), input1(input1_), input2(input2_) {}
  ~Or() = default;
...
public:
  LShift(Wire &input_, size_t number_, Wire &output_)
      : Operation(output_), input(input_), number(number_) {}
  ~LShift() = default;

  bool canBeExecuted() const override { return input.hasSignal(); }
...
public:
  RShift(Wire &input_, size_t number_, Wire &output_)
      : Operation(output_), input(input_), number(number_) {}
  ~RShift() = default;
...
public:
  Attribution(Signal input_, Wire &output_)
      : Operation(output_), input_signal(input_) {}
  Attribution(Wire &input_, Wire &output_)
      : Operation(output_), input_wire(&input_) {}
  ~Attribution() = default;
...
public:
  ...
  void addNewOperation(OperationType type, Wires &wires,
                       Wires &wiresFromInstruction,
  ...
  void run() {
    while (std::any_of(
        ...
        [](const auto &operation) { return !operation->isExecuted(); })) {
...
int main(int argc, char **argv) {
  ...
  foreachLineIn(fileContent, [&wires, &operations](const std::string &line) {
    const auto operationType = getOperationFromInstruction(line);
    auto &wiresFromInstruction = getWiresNameInInstruction(line);
    operations.addNewOperation(operationType, wires, wiresFromInstruction,
                               line);
  });
  ...
  const auto wire =
      std::find_if(std::begin(wiresList), std::end(wiresList),
                   [](const auto wire) { return wire.getName() == "a"; });
  ...
  if (expectedResult == wire->getSignal()) {
    ...
    std::cout << "The result found is " << wire->getSignal()
