...
int main (int argc, char** argv)
{
    ...
    auto aunt = std::find_if(std::begin(aunts), std::end(aunts), [&ticketTape](const auto& aunt)
    {
        ...
        for(auto i = size_t{0}; i < auntCounpounds.size(); ++i)
        {
            ...
            if(auntCounpounds[i].first == "cats" ||
               auntCounpounds[i].first == "trees")
            {
                if(auntCounpounds[i].second.value() <= ticketTape[i].second)
                {
                    return false;
                }
                continue;
            }
            if(auntCounpounds[i].first == "pomeranians" ||
               auntCounpounds[i].first == "goldfish")
            {
                if(auntCounpounds[i].second.value() >= ticketTape[i].second)
                {
                    return false;
