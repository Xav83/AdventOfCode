...
int main (int argc, char** argv)
{
    ...
    const auto winningReindeer = std::max_element(std::begin(reindeers), std::end(reindeers), [](const auto& firstReindeer, const auto& secondReindeer){ return firstReindeer.getCurrentBonusPoint () < secondReindeer.getCurrentBonusPoint (); });
    ...
    {
        std::cout << "The result found is " << pointsOfWinningReindeer << " but the expected result is " << expectedResult << std::endl;
        return 1;
    }
}
