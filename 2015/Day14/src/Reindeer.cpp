...
Reindeer::Reindeer(const std::string &name_, const Speed flyingSpeed_,
                   const Time flyingTime_, const Time restTime_)
    : name(name_), flyingSpeed(flyingSpeed_), flyingTime(flyingTime_),
      restTime(restTime_), timeBeforeChangingState(flyingTime_) {}
...
Reindeer::BonusPoint Reindeer::getCurrentBonusPoint() const {
  return bonusPoints;
}

Reindeer extractInformationFrom(const std::string &line) {
  std::istringstream stream(line);
  std::string reindeerName, can, fly, km_s, for_, seconds, but, then, must,
      rest, for__, seconds_;
  ...
  stream >> reindeerName >> can >> fly >> flyingSpeed >> km_s >> for_ >>
      flyingTime >> seconds >> but >> then >> must >> rest >> for__ >>
      restTime >> seconds_;
