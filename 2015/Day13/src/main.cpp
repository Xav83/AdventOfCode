...
Happiness calculateHappiness(const std::vector<std::reference_wrapper<Person>>& personsPlaced, const std::vector<Relationship>& relationships)
{
    Happiness happiness{0};
...
Happiness gerateTablePosition (std::vector<std::reference_wrapper<Person>>& personsPlaced, std::vector<std::reference_wrapper<Person>>& personsToPlace, const std::vector<Relationship>& relationships)
{
...
int main (int argc, char** argv)
{
    ...
    foreachLineIn(fileContent, [&persons, &relationships](const std::string& line)
    {
        const auto elements = extractInformationFrom (line);
        const auto fromPerson = std::find(std::begin(persons), std::end(persons), Person(std::get<0>(elements)));
        const auto toPerson = std::find(std::begin(persons), std::end(persons), Person(std::get<1>(elements)));
    ...
    {
        std::cout << "The result found is " << optimalHappiness << " but the expected result is " << expectedResult << std::endl;
        return 1;
    }
}
