...
public:
    ...
    std::string toString() const
    {
        ...
        std::generate(result.begin(), result.end(), [index = 0, this] () mutable
        {
            return value[index++];
        });
        return result;
    ...
    bool isSafe () const
    {
        return hasIncreasingStraight() && hasNoConfusingLetters() && hasTwoNonOverlappingPairs();
    }
...
int main (int argc, char** argv)
{
    ...
    {
        std::cout << "The result found is " << newPassword.toString().c_str() << " but the expected result is " << expectedResult.toString().c_str() << std::endl;
        return 1;
    }
}
