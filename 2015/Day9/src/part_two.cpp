...
public:
  ...
  void addCity(const City &city) {
    const auto isNewCity =
        std::end(graph) == std::find(std::begin(graph), std::end(graph), city);
    if (isNewCity) {
  ...
  void addNewRoute(const City &firstCity, Distance distance,
                   const City &secondCity) {
    routes.emplace_back(getStoredCity(firstCity), distance,
                        getStoredCity(secondCity));
  }
...
private:
  Distance travel(const std::vector<Node> &citiesToVisit,
                  Distance distanceTraveledUntilNow,
                  const std::vector<Node> &citiesVisited) {
    auto getRouteDistance = [this, &citiesVisited]() {
      ...
      const auto route = std::find_if(
          ...
          [&citiesVisited](const auto &route) {
            return route.isRouteBetween(
                citiesVisited.back(), citiesVisited[citiesVisited.size() - 2]);
          });
    ...
    for (const auto &city : citiesToVisit) {
      std::vector<Node> citiesVisitedWithNewCity,
          citiesVisitedWithNewCitySorted, lastCitiesToVisit;
      ...
      std::sort(std::begin(citiesVisitedWithNewCitySorted),
                std::end(citiesVisitedWithNewCitySorted));

      std::set_difference(std::begin(sortedGraph), std::end(sortedGraph),
                          std::begin(citiesVisitedWithNewCitySorted),
                          std::end(citiesVisitedWithNewCitySorted),
                          std::back_inserter(lastCitiesToVisit));

      const auto distanceTraveled =
          travel(lastCitiesToVisit, distanceTraveledUntilNow,
                 citiesVisitedWithNewCity);
...
int main(int argc, char **argv) {
  ...
  if (expectedResult == traveledDistance) {
    ...
    std::cout << "The result found is " << traveledDistance
